 ...
 */
 ...
 if($data[file_name1])
 {
  ...
  $new_string = split("/", $new_string);
    ...
    $data[file_name1]=$new_string2.urlencode($new_string[sizeof($new_string)-1]);
	if($s_info[2]>0&&$s_info[2]<4)
	{
		$_xsize = $s_info[0];
		if($_xsize>$_hsize) $_xsize=$_hsize;
		$_srcname = $data[file_name1];
		$_alink="<a href=\"javascript:return false;\" onclick=\"javascript:largePhoto=window.open('$dir/show_pic.php?file=$data[file_name1]','$data[no]','width=$s_info[0],height=$s_info[1],resizable=yes,toolbars=no,scrollbars=auto');largePhoto.focus();\" onfocus=blur()>";
	}
 }
