...
<tr align=center bgcolor=<?=$list_notice_bg_color?> onMouseOver=this.style.backgroundColor='<?=$list_mouse_over_color?>' onMouseOut=this.style.backgroundColor=''>
  <td height=22 class=listnum>:::</td>
<?=$hide_cart_start?><td><input type=checkbox name=cart value="<?=$data[no]?>"></td><?=$hide_cart_end?>
  <td align=left style='word-break:break-all;'><img src=images/t.gif height=3><br>&nbsp;<?=$insert?><?=$icon?><?=$subject?> <font class=commentnum><?=$comment_num?></font></td> 
  <td style='word-break:break-all;'><img src=images/t.gif height=3><br><?=$face_image?> <?=$name?></div></td>
  <td class=listnum><?=$reg_date?></td>
  <td class=listnum><?=$hit?></td>
  <td class=listnum><?=$vote?></td>
</tr>
