...
<tr class=list1>
	<td height=180 valign=top bgcolor=white>
		...
		<tr>
			...
			<td align=right style=padding-right:10px class=list_eng><?=$date?>, 조회수 : <b><?=number_format($hit)?></b></td>
		</tr>
		</table>
		...
		<tr>
			<td>
				...
				<?=$hide_sitelink1_start?><font class=list_eng>- <b>제 작 자</b> : <?=$sitelink1?></font><?=$hide_sitelink1_end?>
				<?=$hide_sitelink2_start?><font class=list_eng> (<?=$sitelink2?>)</font><?=$hide_sitelink2_end?><br>
				<?=$hide_download1_start?><font class=list_eng>- <b>다운로드 </b> : <?=$a_file_link1?><?=$file_name1?> (<?=$file_size1?>)</a>, Download : <?=$file_download1?></font><br><?=$upload_image1?><?=$hide_download1_end?>
